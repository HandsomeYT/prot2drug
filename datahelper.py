...
class DataSet(object):
  ...
  def parse_data(self, FLAGS,  with_label=True): 
    ...
    ligands = json.load(open("/home/xuyt/dc/DeepDTA-master/data/kiba/ligands_can.txt"), object_pairs_hook=OrderedDict)
    proteins = json.load(open("/home/xuyt/dc/DeepDTA-master/data/kiba/proteins.txt"), object_pairs_hook=OrderedDict)

    Y = pickle.load(open("/home/xuyt/dc/DeepDTA-master/data/kiba/Y","rb"), encoding='latin1') ### TODO: read from raw
    if FLAGS.is_log:
        Y = -(np.log10(Y/(math.pow(10,9))))
    #print(one_hot_smiles(ligands['CHEMBL363130'],20,CHARCANSMISET)[0])
    XD = []
    XT = []
